...
def write_tinker_xyz(template, save_path):
    ...
    for i in range(6):
        s += '{:14.8f}'.format(float(template[1, i]))
    s += ' \n'

    # Adding in the coordinate
    for i in range(len(template[2:, 0])):
        s += '{:6d}'.format(int(template[2 + i, 0])) + '{:>4}'.format(str(template[2 + i, 1]))
        s += '{:14.8f}'.format(float(template[2 + i, 2])) + '{:14.8f}'.format(float(template[2 + i, 3])) \
             + '{:14.8f}'.format(float(template[2 + i, 4]))
    ...
    for i, poly in enumerate(inputs['gen_in']['polymorph_num'].split()):
        ...
        for j, T_MD in enumerate(np.array(inputs['temp_in']['temperatures'].split()).astype(float)):
            if T_MD in np.array(inputs['QHA_in']['boltz_temperatures'].split()).astype(float):
                ...
                if os.path.isfile(dirpath + 'PROD.trr'):
                    # Making a directory to store the files in for this temperature
                    if not os.path.isdir(poly + '/boltz_QHA/' + str(j)):
                    ...
                    xyz = np.around(trajectory.xyz[::-1] * 10., 8)
                    ...
                    LD_inputs = {'temperature': [float(T_MD)],
                                 'method': 'HA',
                    ...
                    for k in range(inputs['QHA_in']['max_frames']):
                        ...
                        if not os.path.isdir(frame_dirpath):
                    ...
                    #subprocess.call(['mv', 'hold.trr', poly + '/boltz_QHA/' + str(j)])
