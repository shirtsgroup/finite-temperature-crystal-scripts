...
# All interpolation lines are headed with the keyword INTERPOLATE followed by the number of parameters that will be interpolated.
...
for line in lines:
	...
	if token != 'INTERPOLATE':
	    continue
	numParams = int(tokens[i+1]) #Determine the number of parameters to interpolate
	Params = numpy.zeros([numParams,3],float) #Row 1 is state A, Row 2 is state B, Row 3 is the interpolated state
