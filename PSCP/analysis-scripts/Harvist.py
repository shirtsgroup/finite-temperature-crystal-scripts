...
def CreateTerms(energyfile):
    ...
    if tokens[0] == '#' and tokens[1] == 'This' and tokens[2] == 'file' and tokens[3] == 'was' and \
                    tokens[4] == 'created':
        for line in lines:
            tokens = line.split()
        ...
        Terms = [term.translate(str.maketrans('','','"')) for term in Terms]
...
def GrabTerms(energyfile, Terms='All', ignoreframes=0):
    ...
    ignore_symbols = ['#', '@', '@TYPE', 'STEP', '#Time', 'TIME(ps)', '=====================']
    N = 0
    frames = 0
    for line in lines:
        # Skip header lines
        ...
        # +1 ignores the time in the energy file
        u_nT[N, :] = [float(Tokens[j + 1]) for j in Term_indicies]
        N += 1
...
def PlotFrame(a, u_kln, N_k, xlabel, ylabel, potentials, scat1='None', scat2='None', rescale='yes'):
    ...
    # Set the x and y axis limits by fitting both clouds to a line with a slope of 1 and setting the domain to the
    # interquartile range of the largest cloud
...
def AnimateFrames(f, a, u_klnT, Terms_l, k1t, k2t, Terms1, Terms2, N_k, xlabel='Xaxis', ylabel='Yaxis',
                  potentials=['None', 'None'], type='scat'):
    ...
    aniU = ani.FuncAnimation(fig=f, func=UpdateTimeScat, frames=time, interval=250, fargs=(a, scat1, scat2, u_klnT,
                                                                                           Terms_l, k1t, k2t, Terms1,
                                                                                           Terms2, N_k, xlabel, ylabel,
                                                                                           potentials,type),
                             blit=True)#, init_func=cleanup)
