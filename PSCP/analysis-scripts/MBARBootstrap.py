...
def runMBARBootstrap(u_kln_orig, N_k, beta_k, N, indexVect, datafile, stdfile, n, Header="None", Entropy='No'):
    ...
    # The free energy differences at each bootstrap iteration
    results = np.zeros([num_indicies, iteration_max], float)

    # The rolling standard deviation
    uncertainties = np.zeros([num_indicies, iteration_max], float)
    resultSlice = np.zeros(num_indicies + 1, float)
    uncertaintySlice = np.zeros(num_indicies + 1, float)
    if Entropy=='yes':
        # The average energy difference at each bootstrap iteration
        results_U = np.zeros([num_indicies, iteration_max], float)
        # The rolling standard deviation
        uncertainties_U = np.zeros([num_indicies, iteration_max], float)
        resultSlice_U = np.zeros(num_indicies + 1, float)
        uncertaintySlice_U = np.zeros(num_indicies + 1, float)

        # The entropy differences at each bootstrap iteration
        results_S = np.zeros([num_indicies, iteration_max], float)

        # The rolling standard deviation
        uncertainties_S = np.zeros([num_indicies, iteration_max], float)
        resultSlice_S = np.zeros(num_indicies + 1, float)
        uncertaintySlice_S = np.zeros(num_indicies + 1, float)

    # =============================================================================================
    # ENSURE THE INPUTS ARE CORRECT
    # =============================================================================================
    if len(np.shape(N_k)) == 1 and len(N_k) != len(u_kln[:, 0, 0]):
        print("Number of states in the U_kln matrix: " + str(len(u_kln[:, 0, 0])) + " and in the N_k matrix: " +
              str(len(N_k)) + " are not the same!")
        sys.exit()
    elif int(len(indexVect[:, 0]) != int(2)):
        print("indexVect shape: " + str(np.shape(indexVect)) + " is not valid")
        sys.exit()

    if len(np.shape(N_k)) != 1 and len(N_k[:, 1]) != len(indexVect[0, :]):
        print("N_k shape: " + str(np.shape(N_k)) + " and indexVect shape: " + str(np.shape(indexVect)) +
              " is not valid")
    ...
    for j in range (previous_lines, iteration_max):
        ...
        for s in range(N_ks):
            ...
            if j > previous_lines:
                mbar = pymbar.MBAR(u_kln, N_k_vect, verbose=True, initial_f_k=guess_f_k, method='adaptive',
                                   use_optimized=False)
            else:
        ...
        #    if Entropy=='yes':
        #        for i in range(num_indicies):
        #            resultSlice[i] = np.average(u_kln[indexVect[1,i],indexVect[1,i],:]) - np.average(u_kln[indexVect[0,i],indexVect[0,i],:])
        ...
        for i in range(num_indicies):
            uncertainties[i, j] = np.std(results[i, :(j + 1)])
    ...
    Average = np.zeros(len(results[:, 0]))
    for i in range(num_indicies):
...
def ExtractBootstrap(fname):
    ...
    ddG = np.zeros(len(tokens) - 1, float)
...
def ExtractBootstrapLine(fname, linenum):
    ...
    tokens = line.split()
    dG = np.zeros(len(tokens) - 1, float)
