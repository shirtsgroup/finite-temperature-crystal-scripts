...
if __name__ == '__main__':
    ...
    if (inputs['PSCP_in']['run_restraints'] == True) and (inputs['PSCP_in']['run_interactions'] == True):
        ...
        refT = inputs['PSCP_in']['PSCP_temperature']  
    else:
        Polymorphs, refT, refdG, refddG, refdU, absolutedU = old_systems_dictionary(inputs['gen_in']['potential'], inputs['gen_in']['molecule'])
    ...
    dGvsT(Temperatures=np.array(inputs['temp_in']['temperatures'].split()).astype(int), Pressure=inputs['gen_in']['pressure'],
          ...
          Polymorphs=Polymorphs, refT=refT,
