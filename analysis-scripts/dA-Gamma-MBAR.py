...
if (MINGAMMA == -1 ) and (MAXGAMMA == -1) and (GSPACING == -1) and (exponent == 1):
    ...
    Gammas = ['000L','010L','020L','030L','040L','050L','060L','070L','080L','090L','100L'] #The Gamma points sampled
	...
	if RawGamma < 10:
            ...
            #Gamma_names.append('00' + str(int(Gamma)) + 'G')
	elif RawGamma < 100:
	#elif Gamma < 100:
	    Gamma_names.append('0' + str(int(RawGamma)) + 'G')
            #Gamma_names.append('0' + str(int(Gamma)) + 'G')
	else:
...
# Parameters
T_k = Temp*numpy.ones(len(Gammas),float) #Convert temperatures to floats 
print T_k
print Gammas
#seeds = [201]; #The random seed used (not included at the moment)
g_k = numpy.zeros([len(Gammas)],float)
K = len(Gammas)  # How many states?
if bonds != "no": # total number of states examined; 0 are unsampled if bonds are left on, 1 is unsampled if the bonds are removed
    Kbig=K+1
else:
    Kbig=K
...
for i,poly in enumerate(polymorph):
	...
	for s,hinge in enumerate(hinges):
	    ...
	    dirpath = '/oldhome/ecd4bd/finishedJobs_archive/PSCP/' + Molecule + '/' + Molecule  + '_GRO_' + PotNAME + '_' + polymorph_short[i] + '_' + Molname + Tname + ChargeHinge + '_' + Lname + '_' + Gamma_names[k] + '_' + Pname + hinge
		...
		for counter,line in enumerate(lines):
		    ...
		    u_kln[k,:K,n] = (float(tokens_energy[1]) + numpy.asarray(tokens_dhdl[5:],float))*convert_units[k]
		    dhdl_kn[k,n] = (float(Independent)/Molecules)*(float(tokens_dhdl[2]) + float(tokens_dhdl[3])+ 0.0*float(tokens_dhdl[4]))*convert_units[k]

		    #Use this one if this is a PSCP on a flexible molecule where dihedrals are being turned off
		    #u_kln[k,:K,n] = (float(Independent)/Molecules)*(float(tokens_energy[1]) + numpy.asarray(tokens_dhdl[6:],float))*convert_units[k]
                    #dhdl_kn[k,n] = (float(Independent)/Molecules)*(float(tokens_dhdl[2]) + float(tokens_dhdl[3])+ 1.0*float(tokens_dhdl[4]))*convert_units[k]
